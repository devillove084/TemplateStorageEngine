...
use super::{
    ...
    version::{FILE_META_LENGTH, LevelFileNumIterator, Version},
    version_edit::{FileDelta, FileMetaData, VersionEdit},
};
use crate::{
    cache::table_cache::TableCache,
    compaction::compact::{
        Compaction, CompactionInputs, CompactionReason, CompactionStats, base_range, total_range,
    },
    ...
    manager::filename::{FileType, generate_filename},
...
impl<S: Storage + Clone + 'static, C: Comparator + 'static> VersionSet<S, C> {
        ...
        let (v, encoded_edit) = {
            ...
            if let Some(target_log) = edit.log_number {
                assert!(
                    ...
                    "[version set] applying VersionEdit use a invalid log number {}, expect to be at [{}, {})",
                    target_log,
                    self.log_number,
                    self.next_file_number
                );
    ...
    pub fn create_compaction_output_file(
        &mut self,
