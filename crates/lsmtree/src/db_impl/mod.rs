...
mod tests {
    ...
    use crate::{
        ...
        iterator::Iterator,
        manager::{
            filename::{FileType, parse_filename},
            snapshot::Snapshot,
        },
        memtable::{
        ...
        options::{CompressionType, MAX_KEY_SEQUENCE, Options, ReadOptions, WriteOptions},
        storage::{Storage, mem::MemStorage},
                    ...
                    Some(s) => {
                        ...
                        if bytes.len() > 1000 {
                            if expect != Some(bytes) {
                                panic!(
                                    "expect(len={}), but got(len={}), not equal contents, key: {}, got: {:?}..., expect: {:?}...",
                                    expect.map_or(0, |s| s.len()),
                                    bytes.len(),
                                    k,
                                    &bytes[..50],
                                    &expect.unwrap()[..50]
